...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/7.0/build_glusterfs.sh
...
CURDIR="$(pwd)"
BUILD_DIR="/usr/local"
REPO_URL="https://raw.githubusercontent.com/kbhute-ibm/scripts/master/GlusterFS/7.0/patch/"
...
function runTest() {
	...
	if [[ "$TESTS" == "true" ]]; then
        ...
        if [[ "${ID}" == "ubuntu" ]]; then
			cd "${CURDIR}"
			wget http://rpmfind.net/linux/opensuse/update/leap/42.1/oss/x86_64/gdb-7.11.1-16.1.x86_64.rpm
			rpm2cpio gdb-7.11.1-16.1.x86_64.rpm | cpio -idmv
			mv ./usr/bin/gstack /usr/bin/
			rm -r ./etc ./usr
			rm gdb-7.11.1-16.1.x86_64.rpm
        fi
